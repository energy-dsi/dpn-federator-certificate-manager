...
# three DPN namespaces. The same key+cert is emitted in the format each consumer
...
#   PKCS12 dpn-client.p12 (CN=dpn-client)         -> external mTLS / browser client
#
# Wildcard SANs cover every single-label service under the three DPN namespaces,
...
#   DNS: *.ns-dpn-dp-01.svc.cluster.local
#
...
SAN_DNS_3="*.ns-dpn-dp-01.svc.cluster.local"
...
# ---- Optional extra SANs for the Vault DSM bootstrap CSR only ---------------
# Comma-separated DNS names, e.g.:
#   <<your specific value>>,<<your specific value>>
# Appended AFTER the three fixed wildcard SANs on vault.csr only — does not affect
# the universal server.crt (dpn-common) SANs. The DSM Vault PKI role signing this
# CSR must allow these domains (allowed_domains), or DSM will reject the CSR.
EXTRA_VAULT_SANS="${EXTRA_VAULT_SANS:-}"
...
echo "   SANs         : $SAN_DNS_1, $SAN_DNS_2, $SAN_DNS_3 (wildcard, fixed)"
echo "   O (org)      : $ORG"
...
echo "[7/7] Vault DSM bootstrap CSR ($VAULT_CN, wildcard SAN + extras) -> vault.key + vault.csr..."
cat > "$VAULT_CFG" <<EOF
[ req ]
default_bits       = $KEY_SIZE
prompt             = no
...
# Append any extra SANs (comma-separated) after the three fixed wildcard entries.
if [ -n "$EXTRA_VAULT_SANS" ]; then
  i=4
  OLD_IFS="$IFS"
  IFS=','
  for san in $EXTRA_VAULT_SANS; do
    san=$(printf '%s' "$san" | tr -d ' ')
    if [ -n "$san" ]; then
      echo "DNS.$i = $san" >> "$VAULT_CFG"
      i=$((i + 1))
    fi
  done
  IFS="$OLD_IFS"
